...
//            		params : {start:0, limit:15,recordYearMonth:queryDate}
				...
				proxy: new Ext.data.HttpProxy({
					url : 'develop/count6?t=' + new Date().getTime()
				}),
                remoteSort: false,
...
//					idProperty : 'id',
					...
					{
						name: 'completeAreaHouseSum'
					},
					{
						name: 'completeAreaBusinessSum'
					},
					{
						name: 'completeAreaOfficeSum'
					},
					{
						name: 'completeAreaOtherSum'
					}
					]
				})
            });
        ...
        createGrid: function(id) {
            ...
            this.grid = new Ext.grid.GridPanel({
                ...
                tbar: [' ','张家口市',{
                    ...
                    listeners: {
                    	...
                    	select: function(obj, date) {
                    		this.store.reload({params:{startMonth:document.getElementById("startMonth6").value.replace("-",""),endMonth:document.getElementById("endMonth6").value.replace("-","")}});
                    	},
                ...
                },
                {
                    ...
                    width: 100,
                    //sortable: true,
                    dataIndex: 'completeAreaHouseSum'
                },
                {
                	header: "商业营业房(㎡)",
                	width: 120,
                	//sortable: true,
                	dataIndex: 'completeAreaBusinessSum'
                },
                {
                	header: "办公房(㎡)",
                	width: 100,
                	//sortable: true,
                	dataIndex: 'completeAreaOfficeSum'
                },
                {
                	header: "其他房(㎡)",
                	width: 100,
                	//sortable: true,
                	dataIndex: 'completeAreaOtherSum'
                }],
                bbar:new Ext.Toolbar({
                    buttons: [new Ext.Button({
                        text: '导出到Excel',
                        handler: function() {
                            var vExportContent = Ext.getCmp("gridId6").getExcelXml();
