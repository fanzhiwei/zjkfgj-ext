...
//            		params : {start:0, limit:15,recordYearMonth:queryDate}
				...
				proxy: new Ext.data.HttpProxy({
					url : 'develop/count7?t=' + new Date().getTime()
				}),
                remoteSort: false,
...
//					idProperty : 'id',
					...
					{
						name: 'saledAreaHouseSum'
					},
					{
						name: 'saledAreaBusinessSum'
					},
					{
						name: 'saledAreaOfficeSum'
					},
					{
						name: 'saledAreaOtherSum'
					}
					]
				})
            });
        ...
        createGrid: function(id) {
            ...
            this.grid = new Ext.grid.GridPanel({
                ...
                tbar: [' ','张家口市',{
                    ...
                    listeners: {
                    	...
                    	select: function(obj, date) {
                    		this.store.reload({params:{startMonth:document.getElementById("startMonth7").value.replace("-",""),endMonth:document.getElementById("endMonth7").value.replace("-","")}});
                    	},
                ...
                },
                {
                    ...
                    width: 100,
                    //sortable: true,
                    dataIndex: 'saledAreaHouseSum'
                },
                {
                	header: "商业营业房(㎡)",
                	width: 120,
                	//sortable: true,
                	dataIndex: 'saledAreaBusinessSum'
                },
                {
                	header: "办公房(㎡)",
                	width: 100,
                	//sortable: true,
                	dataIndex: 'saledAreaOfficeSum'
                },
                {
                	header: "其他房(㎡)",
                	width: 100,
                	//sortable: true,
                	dataIndex: 'saledAreaOtherSum'
                }],
                bbar:new Ext.Toolbar({
                    buttons: [new Ext.Button({
                        text: '导出到Excel',
                        handler: function() {
                            var vExportContent = Ext.getCmp("gridId7").getExcelXml();
